...
- 인명·지명·회사명과, 숫자를 글자/자릿수로 표기하는 것은 금지(이미지에 숫자 텍스트가
  찍히는 사고를 막기 위함). 단, 기사에 특정 인원수의 그룹(아이돌 그룹 등)이 등장하면
  key_objects나 action에 그 인원수를 그대로 적어서, 배경 속 단체사진·포스터 등에 인물이
  실제 인원수만큼 그려지게 할 것 (숫자를 글자로 쓰라는 게 아니라 인물 수를 맞추라는 것){avoid_text}"""
...
- 도토리 캐릭터 디자인 규칙 (반드시 지킬 것 — 특히 아래 몸통 형태는 절대 어기지 말 것):
  * 몸통 아래쪽 끝은 반드시 완만한 곡선으로 둥글게 마무리 — 삼각형·물방울·팽이처럼 아래로
    뾰족하게 모이는 형태 절대 금지. 계란/타원을 옆으로 눕힌 듯한 매끈한 둥근 바닥이어야 함
  * 꼬리 절대 금지 — 다람쥐처럼 꼬리를 달지 말 것. 몸통 아래쪽(다리 사이 포함)에서
    아래로 튀어나오거나 돌출되는 형태는 그 어떤 것도(짧은 돌기·뿔·꼭지 모양 포함)
    절대 그리지 말 것 — 다리/발을 제외하면 몸통 바닥은 완전히 매끈하고 막힌 곡면이어야 함
